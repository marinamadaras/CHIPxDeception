...
def test_send_triples(application, monkeypatch, sample_name, sample_sentence):
    dummy_address = 'dummy'
    monkeypatch.setenv("REASONING_ADDRESS", dummy_address)
    with application.app_context(), \
            patch('app.util.extract_triples') as et, \
            patch('app.util.requests') as r:

        send_triples(sample_name, sample_sentence)

        et.assert_called_once_with(sample_name, sample_sentence)
        r.post.assert_called_once_with(AnyStringWith(dummy_address), json=ANY)


def test_extract_triples_no_tokens(application, sample_name, sample_sentence):
    with application.app_context(), patch('app.util.postprocess_triple') as pt, patch('app.util.nltk') as nltk:
        ...
        ret = extract_triples(sample_name, sample_sentence)
        nltk.word_tokenize.assert_called_once_with(sample_sentence)
        pt.assert_not_called()
        assert 'triples' in ret


def test_extract_triples_no_predicate(application, sample_name, sample_sentence, sample_tokens):
    with application.app_context(), patch('app.util.postprocess_triple') as pt, patch('app.util.nltk') as nltk:
...
def test_extract_triples(application, sample_name, sample_sentence, sample_tokens):
    with application.app_context(), patch('app.util.postprocess_triple') as pt, patch('app.util.nltk') as nltk:
